...
def analyze_fit(user_interest: str, papers: list[dict[str, Any]], evidence_confidence: str) -> dict[str, Any]:
    ...
    for paper in visible:
        ...
        topic_similarity = similarity(user_interest, text)
        recency = recency_weight(paper, current_year)
        role_weight = author_role_weight(paper.get("author_role"))
        match_weight = max(0.35, min(1.0, paper.get("match_score", 0.5)))
        review_penalty = 0.78 if paper.get("match_status") == "needs_review" else 1.0
        score = (
            topic_similarity * 0.46
            + recency * 0.18
            + role_weight * 0.14
            + match_weight * 0.18
        ) * review_penalty
        overlap = set(extract_keywords([user_interest], top_k=6)) & set(extract_keywords([text], top_k=8))
        score += min(0.04, len(overlap) * 0.012)
        scored.append((score, paper))
    scored.sort(reverse=True, key=lambda item: item[0])
    best_score = scored[0][0] if scored else 0
    repeated_related = len([score for score, _ in scored if score >= 0.45])
    level = _fit_level(best_score, repeated_related, evidence_confidence, emerging_like)
    related = [_related_item(paper, score) for score, paper in scored[:3] if score >= 0.28]
    matched_keywords = extract_keywords(
        [user_interest] + [_paper_text(paper) for score, paper in scored[:3] if score >= 0.28],
        top_k=7,
    )
    return {
        "fit_level": level,
...
def _fallback_result(user_interest: str) -> dict[str, Any]:
    return {
        "fit_level": "판단 보류",
        "interpretation": "공개 논문 데이터는 제한적입니다. 연구실 소개, 교수소개 페이지, 강의 과목, 현재 프로젝트를 중심으로 컨택 전 확인이 필요합니다.",
...
def _interpretation(level: str, user_interest: str, keywords: list[str], emerging_like: bool) -> str:
    ...
    if level == "판단 보류":
        return f"'{user_interest}'와 연결될 수 있는 공개 근거가 충분하지 않습니다. {keyword_text} 관련 가능성은 연구실 소개와 현재 모집 주제를 통해 확인하는 편이 좋습니다."
    if level == "중간, 확인 필요":
        return f"공개 논문 데이터는 제한적이지만 {keyword_text}와 맞닿을 수 있는 신호가 있습니다. 신임 연구실, 산업체 경력, 프로젝트 키워드가 실제 모집 주제와 연결되는지는 컨택 시 확인이 필요합니다."
    ...
    if level == "중간":
        return f"{keyword_text} 일부가 공개 논문과 맞닿아 있지만, 관심 주제 전체를 직접 다룬다고 보기는 어렵습니다. 관련 프로젝트나 세부 주제 가능성을 확인해볼 만합니다."
    return f"공개 논문 기준으로는 '{user_interest}'와 직접 연결되는 근거가 제한적입니다. 낮은 평가가 아니며, 공개 데이터 밖의 프로젝트나 신임 연구실 방향은 별도 확인이 필요합니다."


def _related_item(paper: dict[str, Any], score: float) -> dict[str, Any]:
    return {
        "id": paper.get("id"),
