...
- `LABFIT_LLM_API_KEY`: 향후 LLM 요약 연동용. 없으면 템플릿 fallback 사용
...
LLM 모듈은 `analysis/llm_trend_summarizer.py`에 분리되어 있습니다. LLM은 정제된 근거를 문장화하는 역할만 하며, 교수님 평가나 논문 진위 판정에는 사용하지 않습니다.
