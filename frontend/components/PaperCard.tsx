...
export function PaperCard({ paper, reason, compact = false }: PaperCardProps) {
  ...
  return (
    <article className="rounded-md border border-warm-gray/20 bg-[#2E2838] p-4 shadow-soft">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h4 className="text-base font-semibold leading-6 text-white">{title}</h4>
          <p className="mt-1 text-sm text-warm-gray">{[year, venue].filter(Boolean).join(" · ") || "학술지/연도 확인 필요"}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {status ? <MatchBadge status={status} /> : null}
          {authorRole ? <span className="rounded-full border border-purple/30 bg-purple/15 px-2.5 py-1 text-xs font-semibold text-purple">{authorRole}</span> : null}
        </div>
      </div>

      {!compact && description ? <p className="mt-3 text-sm leading-6 text-[#F0EDE8]">{description}</p> : null}

      <div className="mt-4 flex flex-wrap gap-2">
        {sourceList.map((source) => (
          <SourceBadge key={source} source={source} />
        ))}
        {!sourceList.length && <span className="text-xs text-warm-gray">출처 확인 필요</span>}
      </div>

      {!compact ? (
        <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-xs font-semibold text-warm-gray">DOI / UCI</dt>
            <dd className="mt-1 break-words text-[#F0EDE8]">{[doi, uci].filter(Boolean).join(" · ") || "확인된 식별자 없음"}</dd>
          </div>
          <div>
            <dt className="text-xs font-semibold text-warm-gray">출처별 인용 신호</dt>
            <dd className="mt-1 text-[#F0EDE8]">
      ...
      {isLinkedPaper && paper.warnings.length ? (
        <div className="mt-4 rounded-md border border-gold/30 bg-gold/10 p-3 text-sm leading-6 text-gold">
          {paper.warnings.join(" ")}
        </div>
      ) : null}

      {url ? (
        <a className="focus-ring mt-4 inline-flex items-center gap-2 rounded-md text-sm font-semibold text-purple hover:text-gold" href={url} target="_blank" rel="noreferrer">
          원문/메타데이터 링크
